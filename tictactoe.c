...
void gameScore(void); //! This will print the game screen with updated moves after every moves.
...
char nextStep(char player) {
	...
	scanf("%u",&col);
	fflush(stdin);
	if((row > 3 || row < 1) || (col > 3 || col < 1)) return player; //if condition fails
	if(TicTacToe[row-1][col-1] == NOTHING) {
		...
		if(--field == 0) return GAMEOVER; //! If all felds are full, game is draw.
